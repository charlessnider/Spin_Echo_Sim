...
function time_propagate_liouville(ρ_list_L, M_list, Mz_list, t0, dt, nsteps, params) 
    ...
    nx = n[1]
    ny = n[2]
    P = convert(Complex{Float64}, 1/nS)
    ...
    t_t = @elapsed begin
        M_eval = [convert(Complex{Float64}, ρ[3]) for ρ in ρ_list_L]
    end
    push!(params["profiling"]["M_eval"], t_t)
    
    t_t = @elapsed begin
        Mz_eval = [(convert(Complex{Float64}, 0.5))*(ρ[1] - ρ[4]) for ρ in ρ_list_L]
    end
    ...
    t_t = @elapsed begin
        M_local = [sum(x.*M_eval) for x in M_stencil_vec]
    end
    push!(params["profiling"]["M_local"], t_t)

    # calculate local magnetization (z)
    t_t = @elapsed begin
        Mz_local = [sum(x.*Mz_eval) for x in M_stencil_vec]
    end
    ...
    for idx = 1:nsteps
        ...
        t_t = @elapsed begin
            ...
            for i = 1:nx
                for j = 1:ny
                    ...
                    # temp hamiltonian
                    tH = zeros(Complex{Float64}, 2, 2)

                    # hard-code hamiltonian
                    tH[1,1] = convert(Complex{Float64}, (1/2)*(ν0 - ν[i,j])) - convert(Complex{Float64}, (αz/2)*t_Mz)
                    tH[1,2] = -convert(Complex{Float64}, (αy/4)*(conj(t_Mp) - t_Mp*exp(-2im*ν0*t))) - convert(Complex{Float64}, (αx/4)*(conj(t_Mp) + t_Mp*exp(-2im*ν0*t)))
                    tH[2,1] = -convert(Complex{Float64}, (αy/4)*(t_Mp - conj(t_Mp)*exp(2im*ν0*t))) - convert(Complex{Float64}, (αx/4)*(t_Mp + conj(t_Mp)*exp(2im*ν0*t)))
                    tH[2,2] = convert(Complex{Float64}, (1/2)*(ν[i,j] - ν0)) + convert(Complex{Float64}, (αz/2)*t_Mz)
        ...
        # calculate propagators, adding in dissipation & then convert back to Float64
        t_t = @elapsed begin
            U_L = [convert.(Complex{Float64}, exp(( -1im*H + J_L )*dt)) for H in H_L]
        end
...
function shift_stencil(params)
    ...
    bigS = Array{Any}(undef, nx, ny)

    # do the shift
    for v in spin_idx
        shift = (spin_idx[v...][1] - 1, spin_idx[v...][2] - 1)
        temp = circshift(t_stencil, shift)
        bigS[v[1], v[2]] = temp
    end
